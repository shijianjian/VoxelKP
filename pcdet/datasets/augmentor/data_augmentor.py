...
class DataAugmentor(object):
    ...
    def random_local_noise(self, data_dict=None, config=None):
        ...
        prob = config.get("PROBABILITY", 1.)
        if np.random.random() > prob:
            return data_dict

        jittering_ratio = config['JITTERING_RATIO']
        point_ratio = config['POINT_RATIO']
        gt_boxes, points = augmentor_utils.local_add_random_noise(
            data_dict['gt_boxes'], data_dict['points'], jittering_ratio, point_ratio)
        data_dict['gt_boxes'] = gt_boxes
        data_dict['points'] = points
        return data_dict

    def random_local_inter_object_noise(self, data_dict=None, config=None):
        if data_dict is None:
            return partial(self.random_local_inter_object_noise, config=config)
        ...
        point_ratio = config['POINT_RATIO']
        gt_boxes, points = augmentor_utils.local_add_inter_object_noise(
            data_dict['gt_boxes'], data_dict['points'], point_ratio)
        data_dict['gt_boxes'] = gt_boxes
        data_dict['points'] = points
        return data_dict
    ...
    def forward(self, data_dict):
        ...
        if 'gt_boxes_mask' in data_dict:
            ...
            if 'keypoint_location' in data_dict:
                ...
                data_dict['keypoint_visibility'] = data_dict['keypoint_visibility'][gt_boxes_mask]
                data_dict['keypoint_mask'] = data_dict['keypoint_mask'][gt_boxes_mask]
