...
def local_add_random_noise(gt_boxes, points, jittering_ratio, point_ratio):
    """
    Args:
        gt_boxes: (N, 7), [x, y, z, dx, dy, dz, heading, [vx], [vy]]
        points: (M, 3 + C),
    ...
    for idx, box in enumerate(gt_boxes):
        x, y, z, dx, dy, dz = box[0], box[1], box[2], box[3], box[4], box[5]
        jitter_size = np.array([dx, dy, dz, 0, 0])[None] * jittering_ratio
        points_in_box, mask = get_points_in_box(points, box)
        noise = points_in_box.copy()
        np.random.shuffle(noise)
        noise = noise[:int(len(noise) * point_ratio)]
        noise = noise + np.random.randn(*noise.shape) * jitter_size
        all_noise.append(noise)
    points = np.concatenate([points, *all_noise])

    return gt_boxes, points


def local_add_inter_object_noise(gt_boxes, points, point_ratio):
    """
    Args:
        gt_boxes: (N, 7), [x, y, z, dx, dy, dz, heading, [vx], [vy]]
        points: (M, 3 + C),
    ...
    shuffle_indices = np.array(range(len(gt_boxes)))
    np.random.shuffle(shuffle_indices)

    all_noise = []
    for idx, (box_1, box_2) in enumerate(zip(gt_boxes, gt_boxes[shuffle_indices])):
        x, y, z, dx, dy, dz = box_1[0], box_1[1], box_1[2], box_1[3], box_1[4], box_1[5]
        x2, y2, z2, dx2, dy2, dz2 = box_2[0], box_2[1], box_2[2], box_2[3], box_2[4], box_2[5]
        points_in_box, mask = get_points_in_box(points, box_1)
        noise = points_in_box.copy()
        np.random.shuffle(noise)
        noise = noise[:int(len(noise) * point_ratio)] - np.array([x, y, z, 0, 0])[None] + np.array([x2, y2, z2, 0, 0])[None]
        all_noise.append(noise)
