...
from ..backbones_3d.spconv_backbone import SparseLayerNorm, SparseAttentionBlock
import copy
...
class SparseMBConv2d(spconv.SparseModule):
    expansion = 1

    def __init__(self, inplanes, planes, stride=1, norm_fn=None, downsample=None, indice_key=None, mid_channels=None):
        super(SparseMBConv2d, self).__init__()
        mid_channels = mid_channels or round(inplanes * self.expansion)

        assert norm_fn is not None
        bias = norm_fn is not None
        self.inverted_conv = spconv.SubMConv2d(
            inplanes, mid_channels, kernel_size=1, stride=1, padding=0, bias=bias, indice_key=indice_key
        )
        self.depth_conv = spconv.SubMConv2d(
            mid_channels, mid_channels, kernel_size=3, stride=stride, padding=1, bias=bias, indice_key=indice_key
        )
        self.bn1 = norm_fn(planes)
        self.relu = nn.Hardswish()
        self.point_conv = spconv.SubMConv2d(
            mid_channels, planes, kernel_size=1, stride=1, padding=0, bias=bias, indice_key=indice_key
        )
...
class SeparateHeadMBConv(nn.Module):
    def __init__(self, input_channels, sep_head_dict, kernel_size, init_bias=-2.19, use_bias=False):
        super().__init__()
        self.sep_head_dict = sep_head_dict
        ...
        for cur_name in self.sep_head_dict:
            ...
            for k in range(num_conv - 1):
                ...
                #     nn.ReLU()
                # ))
                fc_list.append(spconv.SparseSequential(SparseMBConv2d(input_channels, input_channels, norm_fn=nn.BatchNorm1d, indice_key=cur_name)))
            fc_list.append(spconv.SubMConv2d(input_channels, output_channels, 1, bias=True, indice_key=cur_name+'out'))
            fc = nn.Sequential(*fc_list)
            if 'hm' in cur_name:
                fc[-1].bias.data.fill_(init_bias)
...
class SeparateHeadLN(nn.Module):
    def __init__(self, input_channels, sep_head_dict, kernel_size, init_bias=-2.19, use_bias=False):
        super().__init__()
        self.sep_head_dict = sep_head_dict
        ...
        for cur_name in self.sep_head_dict:
            ...
            for k in range(num_conv - 1):
                fc_list.append(spconv.SparseSequential(
                    spconv.SubMConv2d(input_channels, input_channels, kernel_size, padding=int(kernel_size//2), bias=use_bias, indice_key=cur_name),
                    SparseLayerNorm(input_channels),
                    nn.GELU()
                ))
            fc_list.append(spconv.SubMConv2d(input_channels, output_channels, 1, bias=True, indice_key=cur_name+'out'))
            fc = nn.Sequential(*fc_list)
...
class SeparateHead3DAttention(nn.Module):
    def __init__(self, input_channels, sep_head_dict, kernel_size, init_bias=-2.19, use_bias=False):
        super().__init__()
        self.sep_head_dict = sep_head_dict
        ...
        for cur_name in self.sep_head_dict:
            ...
            for k in range(num_conv - 1):
                fc_list.append(spconv.SparseSequential(
                    # SparseAttentionBlock(input_channels, num_heads=input_channels // 64, indice_key=cur_name + '_att', window_size=6, dropout=0.),
                    spconv.SubMConv3d(input_channels, input_channels, kernel_size, padding=int(kernel_size//2), bias=use_bias, indice_key=cur_name),
                    SparseLayerNorm(input_channels),
                    nn.GELU()
                ))
            fc_list.append(spconv.SubMConv3d(input_channels, output_channels, 1, bias=True, indice_key=cur_name + 'out'))
            fc = nn.Sequential(*fc_list)
            if 'hm' in cur_name:
                fc[-1].bias.data.fill_(init_bias)
            else:
...
class SeparateHead3D(nn.Module):
    def __init__(self, input_channels, sep_head_dict, kernel_size, init_bias=-2.19, use_bias=False):
        super().__init__()
        self.sep_head_dict = sep_head_dict
...
# Make sure the input for the forward is SparseConvTensor.
class SparseAttention(spconv.SparseModule):

    def __init__(self, input_channels, n_heads, dropout=0.):
        super().__init__()

        self.input_channels = input_channels
        # It is actually the length of score matrix
        self.n_heads = n_heads
        self.d_k = input_channels // n_heads

        self.q_op = spconv.SubMConv2d(input_channels, input_channels, 3, stride=1, padding=1, bias=False)
        self.k_op = spconv.SubMConv2d(input_channels, input_channels, 3, stride=1, padding=1, bias=False)
        self.v_op = spconv.SubMConv2d(input_channels, input_channels, 3, stride=1, padding=1, bias=False)

        self.dropout = nn.Dropout(dropout)
        # self.out = spconv.SubMConv2d(input_channels, input_channels, 3, stride=1, padding=1, bias=True)
        # Initialize weights  
        self.q_op.weight.data = torch.nn.init.xavier_uniform_(self.q_op.weight.data)

    def sparse_attention(self, q: spconv.SparseConvTensor, k: spconv.SparseConvTensor, v: spconv.SparseConvTensor):
        output_tensor = torch.zeros_like(q.features)
        indices = torch.arange(q.indices.size(0), device=q.indices.device, dtype=q.indices.dtype)
        for num in range(q.batch_size):
            # Reshape and transpose
            q_one = q.features[q.indices[:, 0] == num].view(1, -1, self.n_heads, self.d_k)
            k_one = k.features[k.indices[:, 0] == num].view(1, -1, self.n_heads, self.d_k)
            v_one = v.features[v.indices[:, 0] == num].view(1, -1, self.n_heads, self.d_k)
            # Attention
            # (1, n_points, self.n_heads, self.n_heads)
            scores = torch.matmul(q_one, k_one.transpose(-2, -1)) / math.sqrt(self.d_k)
            scores = F.softmax(scores, dim=-1)
            scores = self.dropout(scores)
            output = torch.matmul(scores, v_one)
            output = output.reshape(q.features[q.indices[:, 0] == num].size(0,), q.features.size(1))
            output_tensor.index_add_(0, indices[q.indices[:, 0] == num], output)

        return spconv.SparseConvTensor(
            features=output_tensor,
            indices=q.indices,
            spatial_shape=q.spatial_shape,
            batch_size=q.batch_size
        )

    def pos_embedding(self, sparse_tensor: spconv.SparseConvTensor, normalize: bool = False) -> torch.Tensor:
        z_size, y_size, x_size = sparse_tensor.spatial_shape
        batch_size = sparse_tensor.batch_size

        sample_target = torch.arange(
            z_size * y_size * x_size, device=sparse_tensor.indices.device, dtype=sparse_tensor.indices.dtype
        ).reshape(z_size, y_size, x_size)
        z, y, x = sparse_tensor.indices[:, 1].long(), sparse_tensor.indices[:, 2].long(), sparse_tensor.indices[:, 3].long()
        indices = torch.arange(sparse_tensor.indices.size(0), device=sparse_tensor.indices.device, dtype=sparse_tensor.indices.dtype)
        pos_embedding = torch.zeros_like(sparse_tensor.indices[:, 0])

        for b in range(batch_size):
            # grid_z, grid_y, grid_x = torch.meshgrid(z[sparse_tensor.indices[:, 0].cpu() == b], y[sparse_tensor.indices[:, 0].cpu() == b], x[sparse_tensor.indices[:, 0].cpu() == b])
            # grid = torch.stack([grid_z, grid_y, grid_x])[None]
            b_pos_embedding = sample_target[z[sparse_tensor.indices[:, 0] == b], y[sparse_tensor.indices[:, 0] == b], x[sparse_tensor.indices[:, 0] == b]]
            pos_embedding.index_add_(0, indices[sparse_tensor.indices[:, 0] == b], b_pos_embedding.reshape(-1))

        if normalize:
            pos_embedding = pos_embedding / (z_size * y_size * x_size) * 2 - 1

        return pos_embedding

    def pos_embedding_2d(self, sparse_tensor: spconv.SparseConvTensor, normalize: bool = False) -> torch.Tensor:
        y_size, x_size = sparse_tensor.spatial_shape
        batch_size = sparse_tensor.batch_size

        sample_target = torch.arange(
            y_size * x_size, device=sparse_tensor.indices.device, dtype=sparse_tensor.indices.dtype
        ).reshape(y_size, x_size)
        indices = torch.arange(sparse_tensor.indices.size(0), device=sparse_tensor.indices.device, dtype=sparse_tensor.indices.dtype)
        y, x = sparse_tensor.indices[:, 1].long(), sparse_tensor.indices[:, 2].long()
        pos_embedding = torch.zeros_like(sparse_tensor.indices[:, 0])

        for b in range(batch_size):
            # grid_z, grid_y, grid_x = torch.meshgrid(z[sparse_tensor.indices[:, 0].cpu() == b], y[sparse_tensor.indices[:, 0].cpu() == b], x[sparse_tensor.indices[:, 0].cpu() == b])
            # grid = torch.stack([grid_z, grid_y, grid_x])[None]
            b_pos_embedding = sample_target[y[sparse_tensor.indices[:, 0] == b], x[sparse_tensor.indices[:, 0] == b]]
            pos_embedding.index_add_(0, indices[sparse_tensor.indices[:, 0] == b], b_pos_embedding.reshape(-1))

        if normalize:
            pos_embedding = pos_embedding / (y_size * x_size) * 2 - 1

        return pos_embedding

    def forward(self, q: spconv.SparseConvTensor, k: Optional[spconv.SparseConvTensor] = None, v: Optional[spconv.SparseConvTensor] = None):
        if k is None and v is None:
            k = v = q
        # Sparse projection
        k = self.k_op(k)

        q = q.replace_feature(q.features + self.pos_embedding_2d(q)[:, None])
        q = self.q_op(q)
        v = self.v_op(v)
...
class SeparateAttentionHead(nn.Module):
    def __init__(self, input_channels, sep_head_dict, kernel_size, init_bias=-2.19, use_bias=False):
        super().__init__()
        self.sep_head_dict = sep_head_dict
        ...
        for cur_name in self.sep_head_dict:
            ...
            for k in range(num_conv - 1):
                fc_list.append(spconv.SparseSequential(
                    SparseAttention(input_channels, n_heads=16),
                    spconv.SubMConv2d(input_channels, input_channels, kernel_size, padding=int(kernel_size // 2), bias=use_bias, indice_key=cur_name),
                    nn.BatchNorm1d(input_channels),
                    nn.ReLU()
                ))
...
class VoxelNeXtHead(nn.Module):
    def __init__(self, model_cfg, input_channels, num_class, class_names, grid_size, point_cloud_range, voxel_size,
        ...
        assert total_classes == len(self.class_names), f'class_names_each_head={self.class_names_each_head}, class_names={self.class_names}'

        kernel_size_head = self.model_cfg.get('KERNEL_SIZE_HEAD', 3)
        head_module = self.model_cfg.get('HEAD_MODULE', "SeparateHead")
        if head_module == "SeparateHead":
            head = SeparateHead
        elif head_module == "SeparateHead3DAttention":
            head = SeparateHead3DAttention
        elif head_module == "SeparateHead3D":
            head = SeparateHead3D
        elif head_module == "SeparateAttentionHead":
            head = SeparateAttentionHead
        elif head_module == "SeparateHeadMBConv":
            head = SeparateHeadMBConv
        else:
            raise ValueError
        ...
        for idx, cur_class_names in enumerate(self.class_names_each_head):
            ...
            cur_head_dict['hm'] = dict(out_channels=self.model_cfg.get('HEATMAP_OUT_CHANNELS', len(cur_class_names)), num_conv=self.model_cfg.NUM_HM_CONV)
