...
def neg_loss_sparse_dynamic(pred, gt):
    """
    Refer to https://github.com/tianweiy/CenterPoint.
    Modified focal loss. Exactly the same as CornerNet. Runs faster and costs a little bit more memory
    Args:
    ...
    Returns:
    """
    max_value = gt.max(dim=0)[0]

    gt_norm = gt / max_value[None]

    pos_inds = gt_norm.eq(1).float()
    neg_inds = gt_norm.lt(1).float()

    neg_weights = torch.pow(1 - gt_norm, 4)

    loss = 0

    pred_sigmoid = torch.clamp((pred / max_value[None]).sigmoid(), min=1e-4, max=1 - 1e-4)
    ...
    pos_loss = pos_loss.mean(dim=0).sum()
    neg_loss = neg_loss.mean(dim=0).sum()
...
class FocalLossSparseDynamic(nn.Module):
    """
    Refer to https://github.com/tianweiy/CenterPoint
    """
    def __init__(self):
        super(FocalLossSparseDynamic, self).__init__()
        self.neg_loss = neg_loss_sparse_dynamic
